...
function mapPost(row: typeof tableBlogPosts.$inferSelect, categorySlug: string): BlogPost {
  return {
    id: String(row.id),
...
export function getAffiliateShowBlogBlockEnabled(): boolean {
  const db = getDb()
  const row = db.select({ v: tableSiteSettings.affiliate_show_blog_block }).from(tableSiteSettings).limit(1).get()
  if (row?.v == null) return true
  return row.v === true
}
...
export function getBlogCategoriesSorted(): BlogCategory[] {
  const db = getDb()
  const rows = db.select().from(tableBlogCategories).where(isNull(tableBlogCategories.deleted_at)).orderBy(asc(tableBlogCategories.order_index), asc(tableBlogCategories.id)).all()
  return rows.map(mapCategory)
}

export function getPublishedPosts(categorySlug?: string | null): BlogPost[] {
  const db = getDb()
  const conditions = [eq(tableBlogPosts.status, "published"), isNull(tableBlogPosts.deleted_at), isNull(tableBlogCategories.deleted_at)]
...
export function getPostBySlugs(categorySlug: string, postSlug: string): BlogPost | undefined {
  ...
  const row = db
    ...
    .from(tableBlogPosts)
    .innerJoin(tableBlogCategories, eq(tableBlogPosts.category_id, tableBlogCategories.id))
    .where(and(eq(tableBlogCategories.slug, categorySlug), eq(tableBlogPosts.slug, postSlug), eq(tableBlogPosts.status, "published"), isNull(tableBlogPosts.deleted_at), isNull(tableBlogCategories.deleted_at)))
    .get()
