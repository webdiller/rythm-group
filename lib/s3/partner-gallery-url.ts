...
export function isPartnerGalleryOriginalKey(value: string | null | undefined): boolean {
  ...
  const v = value.trim()
  return (
    v.startsWith(PARTNER_GALLERY_KEY_PREFIX) &&
    v.includes("/gallery/") &&
...
export function parsePartnerCaseGalleryJson(
  raw: string | null | undefined,
): PartnerCaseGalleryImage[] {
  if (!raw?.trim()) return []
  try {
    ...
    for (const item of parsed) {
      ...
      const thumbnailWidth =
        typeof row.thumbnailWidth === "number" ? row.thumbnailWidth : Number(row.thumbnailWidth)
      const thumbnailHeight =
        typeof row.thumbnailHeight === "number" ? row.thumbnailHeight : Number(row.thumbnailHeight)
...
export function getPartnerGalleryThumbnailSrc(
