...
import { type PartnerCaseGalleryImage, isPartnerGalleryOriginalKey, isPartnerGalleryThumbnailKey, parsePartnerCaseGalleryJson, serializePartnerCaseGallery } from "@/lib/s3/partner-gallery-url"

export { type PartnerCaseGalleryImage, getPartnerGalleryOriginalSrc, getPartnerGalleryThumbnailSrc, isPartnerGalleryOriginalKey, isPartnerGalleryThumbnailKey, parsePartnerCaseGalleryJson, serializePartnerCaseGallery } from "@/lib/s3/partner-gallery-url"
...
async function encodeThumbnailUnderLimit(input: Buffer): Promise<{
	...
	const fallback = await sharp(input).rotate().resize(160, 160, { fit: "inside", withoutEnlargement: true }).jpeg({ quality: 40, mozjpeg: true }).toBuffer()
...
export async function uploadPartnerGalleryImage(partnerId: number, inputBuffer: Buffer): Promise<PartnerCaseGalleryImage> {
	const imageId = randomUUID()
	const originalPipeline = sharp(inputBuffer).rotate().resize(ORIGINAL_MAX_LONG_SIDE, ORIGINAL_MAX_LONG_SIDE, {
		fit: "inside",
		withoutEnlargement: true,
	})
...
export async function deletePartnerGalleryImageObjects(image: Pick<PartnerCaseGalleryImage, "originalKey" | "thumbnailKey">): Promise<void> {
...
export async function deletePartnerCaseGalleryAll(raw: string | null | undefined): Promise<void> {
