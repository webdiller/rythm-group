...
import { BLOG_IMAGES_KEY_PREFIX, BLOG_VIDEOS_KEY_PREFIX, isBlogImageS3Key, isBlogS3Key, isBlogVideoS3Key, tryExtractBlogKeyFromPublicUrl } from "@/lib/s3/blog-asset-url"
import { deleteBlogUploadFileByPublicUrl, deleteBlogVideoFileByPublicUrl } from "@/lib/blog-local-upload"
import { isLocalBlogUploadUrl, isLocalBlogVideoUploadUrl } from "@/lib/blog/local-upload-url"

export { BLOG_IMAGES_KEY_PREFIX, BLOG_VIDEOS_KEY_PREFIX, getBlogCoverSrc, isBlogImageS3Key, isBlogS3Key, isBlogVideoS3Key, isManagedBlogImageRef, isManagedBlogVideoRef } from "@/lib/s3/blog-asset-url"
...
export async function uploadBlogImage(buf: Buffer, mime: string): Promise<{ key: string; url: string }> {
  const key = buildBlogImageKey(mime)
  await putPublicObject({ key, body: buf, contentType: mime })
  return { key, url: getPublicObjectUrl(key) }
}

export async function uploadBlogVideo(buf: Buffer, mime: string): Promise<{ key: string; url: string }> {
...
export function extractBlogKeyFromUrl(url: string): string | null {
  ...
  try {
    ...
    const base = getPublicObjectUrlFromBase(`${YA_ENDPOINT.replace(/\/$/, "")}/${YA_BUCKET_NAME}`, "").replace(/\/$/, "")
