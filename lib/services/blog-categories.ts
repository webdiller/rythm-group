...
export class ServiceBlogCategories {
	static listForAdmin(includeDeleted: boolean) {
		...
		const rows = includeDeleted ? q.orderBy(asc(tableBlogCategories.order_index), asc(tableBlogCategories.id)).all() : q.where(isNull(tableBlogCategories.deleted_at)).orderBy(asc(tableBlogCategories.order_index), asc(tableBlogCategories.id)).all()
	...
	static patch(id: number, body: PatchCategoryBody) {
		...
		if (body.slug !== undefined && body.slug !== existing.slug) {
			const clash = db
				.select({ id: tableBlogCategories.id })
				.from(tableBlogCategories)
				.where(and(eq(tableBlogCategories.slug, body.slug), isNull(tableBlogCategories.deleted_at), ne(tableBlogCategories.id, id)))
				.get()
