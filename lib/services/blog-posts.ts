...
export class ServiceBlogPosts {
	...
	static assertSlugFreeInCategory(categoryId: number, slug: string, exceptPostId?: number) {
		const db = getDb()
		const parts = [eq(tableBlogPosts.category_id, categoryId), eq(tableBlogPosts.slug, slug), isNull(tableBlogPosts.deleted_at)] as const
	...
	static async patch(id: number, body: PatchPostBody, rawBody?: Record<string, unknown>) {
		...
		const coverKeyPresent = rawBody != null && Object.prototype.hasOwnProperty.call(rawBody, "cover_image_url")
		...
		const oldPosterUrls = new Set<string>([...extractVideoPosterUrlsFromHtml(existing.body_html_ru), ...extractVideoPosterUrlsFromHtml(existing.body_html_en)])
		const nextBodyRu = safeRu ?? existing.body_html_ru
		const nextBodyEn = safeEn ?? existing.body_html_en
		const nextPosterUrls = new Set<string>([...extractVideoPosterUrlsFromHtml(nextBodyRu), ...extractVideoPosterUrlsFromHtml(nextBodyEn)])
	...
	static async softDelete(id: number) {
		...
		const ts = unixNow()
		db.update(tableBlogPosts).set({ deleted_at: ts, updated_at: isoNow() }).where(eq(tableBlogPosts.id, id)).run()
