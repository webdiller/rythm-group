...
import type { GetAllResponse, GetOneParams, GetOneResponse, CreateOneBody, CreateOneResponse, UpdateOneParams, UpdateOneBody, UpdateOneResponse, DeleteOneParams, DeleteOneResponse } from "@/lib/schemas/channels"
...
export class ServiceChannels {
  ...
  static async deleteOne(params: DeleteOneParams): Promise<DeleteOneResponse> {
    ...
    const id = Number(params.id)
    if (Number.isNaN(id)) throw new Error("Invalid id")
    const existing = db.select({ avatar: tableChannels.avatar }).from(tableChannels).where(eq(tableChannels.id, id)).get()
