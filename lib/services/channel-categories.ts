...
import type { GetAllResponse, GetOneParams, GetOneResponse, CreateOneBody, CreateOneResponse, UpdateOneParams, UpdateOneBody, UpdateOneResponse, DeleteOneParams, DeleteOneResponse } from "@/lib/schemas/channel-categories"
...
export class ServiceChannelCategories {
  ...
  static updateOne(params: UpdateOneParams, body: UpdateOneBody): UpdateOneResponse {
    ...
    const [updated] = db.update(tableChannelCategories).set(set).where(eq(tableChannelCategories.id, params.id)).returning().all()
  ...
  static deleteOne(params: DeleteOneParams): DeleteOneResponse {
    ...
    db.update(tableChannels).set({ category_id: null }).where(eq(tableChannels.category_id, params.id)).run()
