...
import { BACKGROUND_SLOTS, type BackgroundKind, type BackgroundSlot, type BackgroundsMap, backgroundSlotToFilename, getBackgroundSrc, parseBackgroundsJson, resolveBackgroundSlot, serializeBackgroundsMap, isBackgroundS3Key } from "@/lib/s3/background-slots"
...
export function getBackgroundKey(kind: BackgroundKind, theme?: string | null, scope?: string | null): string | null {
...
export function setBackgroundKey(slot: BackgroundSlot, key: string | null): void {
	...
	if (existing) {
		db.update(tableSiteSettings).set({ backgrounds: json }).where(eq(tableSiteSettings.id, existing.id)).run()
		return
	}
	db.insert(tableSiteSettings).values({ backgrounds: json }).run()
}

export async function replaceBackgroundKey(slot: BackgroundSlot, nextKey: string): Promise<void> {
...
export function resolveBackgroundSrc(kind: BackgroundKind, theme?: string | null, scope?: string | null): string {
	const slot = resolveBackgroundSlot(kind, theme, scope)
...
export async function getBackgroundPresenceMap(): Promise<Record<BackgroundSlot, boolean>> {
	const entries = await Promise.all(BACKGROUND_SLOTS.map(async (slot) => [slot, await hasBackgroundSlot(slot)] as const))
...
export async function hasGlobalBackgroundThemes(scope?: "affiliate" | null): Promise<{
	...
	const [light, dark] = await Promise.all([hasBackgroundSlot(lightSlot), hasBackgroundSlot(darkSlot)])
