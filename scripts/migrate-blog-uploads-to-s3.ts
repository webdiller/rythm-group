...
async function main() {
  ...
  console.log(
    `[migrate-blog-uploads] bucket=${env.YA_BUCKET_NAME} region=${env.YA_REGION} endpoint=${env.YA_ENDPOINT}`,
  )
  ...
  const db = getDb()
  const rows = db
    .select({ id: tableBlogPosts.id, cover_image_url: tableBlogPosts.cover_image_url })
    .from(tableBlogPosts)
    .all()
  ...
  console.log(
    `[migrate-blog-uploads] done. files uploaded=${uploaded} skipped=${skippedFiles} failed=${failedFiles}; covers migrated=${coversMigrated} skipped=${coversSkipped}`,
  )
