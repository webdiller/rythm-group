...
import { isChannelAvatarS3Key, isLegacyBase64ChannelAvatar, uploadChannelAvatarWebp } from "@/lib/s3/channel-avatar"
import { getYaStorageEnv } from "@/lib/s3/env"

async function main() {
	const env = getYaStorageEnv()
	console.log(`[migrate-channel-avatars] bucket=${env.YA_BUCKET_NAME} region=${env.YA_REGION} endpoint=${env.YA_ENDPOINT}`)

	const db = getDb()
	const rows = db.select({ id: tableChannels.id, name: tableChannels.name, avatar: tableChannels.avatar }).from(tableChannels).all()
	...
	for (const row of rows) {
		...
		try {
			...
			db.update(tableChannels).set({ avatar: key }).where(eq(tableChannels.id, row.id)).run()
			migrated += 1
	...
	console.log(`[migrate-channel-avatars] done. migrated=${migrated} skipped=${skipped} failed=${failed} total=${rows.length}`)
