...
async function main() {
  ...
  console.log(
    `[migrate-site-branding] bucket=${env.YA_BUCKET_NAME} region=${env.YA_REGION} endpoint=${env.YA_ENDPOINT}`,
  )
  ...
  console.log(
    `[migrate-site-branding] done. migrated=${migrated} skipped=${skipped} failed=${failed}`,
  )
