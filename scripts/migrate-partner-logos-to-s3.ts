...
async function main() {
  ...
  console.log(
    `[migrate-logos] bucket=${env.YA_BUCKET_NAME} region=${env.YA_REGION} endpoint=${env.YA_ENDPOINT}`,
  )

  const db = getDb()
  const rows = db
    .select({ id: tablePartners.id, name: tablePartners.name, logo_url: tablePartners.logo_url })
    .from(tablePartners)
    .all()
  ...
  for (const row of rows) {
    ...
    try {
      ...
      db.update(tablePartners)
        .set({ logo_url: key })
        .where(eq(tablePartners.id, row.id))
        .run()
      migrated += 1
  ...
  console.log(
    `[migrate-logos] done. migrated=${migrated} skipped=${skipped} failed=${failed} total=${rows.length}`,
  )
