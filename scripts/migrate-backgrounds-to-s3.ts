...
import {
  ...
  type BackgroundsMap,
  uploadBackgroundWebp,
} from "@/lib/s3/backgrounds"
import { legacyBackgroundDiskPath } from "@/lib/server/site-backgrounds"

async function main() {
  const env = getYaStorageEnv()
  console.log(
    `[migrate-backgrounds] bucket=${env.YA_BUCKET_NAME} region=${env.YA_REGION} endpoint=${env.YA_ENDPOINT}`,
  )
  ...
  for (const slot of BACKGROUND_SLOTS) {
    ...
    try {
      ...
      skipped += 1
      console.log(
        `[migrate-backgrounds] ${slot}: no file ${backgroundSlotToFilename(slot)} — skip`,
      )
      continue
    }
  ...
  console.log(
    `[migrate-backgrounds] done. migrated=${migrated} skipped=${skipped} failed=${failed}`,
  )
