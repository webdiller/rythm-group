...
async function main() {
  ...
  console.log(
    `[rewrite-blog-html] bucket=${env.YA_BUCKET_NAME} public via ${env.YA_ENDPOINT}/${env.YA_BUCKET_NAME}`,
  )
  ...
  console.log(
    `[rewrite-blog-html] done. posts=${postsTouched} replacements=${totalReplacements} scanned=${rows.length}`,
  )
}
