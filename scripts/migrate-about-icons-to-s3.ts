...
async function main() {
  ...
  console.log(
    `[migrate-about-icons] bucket=${env.YA_BUCKET_NAME} region=${env.YA_REGION} endpoint=${env.YA_ENDPOINT}`,
  )
  ...
  for (const row of rows) {
    ...
    try {
      ...
      db.update(tableAboutCards)
        .set({ icon_image: key })
        .where(eq(tableAboutCards.id, row.id))
        .run()
      migrated += 1
  ...
  console.log(
    `[migrate-about-icons] done. migrated=${migrated} skipped=${skipped} failed=${failed} total=${rows.length}`,
  )
