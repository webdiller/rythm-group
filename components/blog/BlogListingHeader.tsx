...
export function BlogListingHeader({ category }: BlogListingHeaderProps) {
	...
	if (category) {
		...
		return (
			<header
				id="top"
				className="mb-8 space-y-4 lg:mb-10 lg:space-y-5"
			>
				<StaggerItem
					index={0}
					delayStart={120}
				...
				>
					<div className="space-y-3">
						<h1 className="font-(family-name:--font-space-grotesk) text-3xl font-bold tracking-tight text-foreground sm:text-4xl md:text-5xl">{title}</h1>
					</div>
				</StaggerItem>
			</header>
		)
	...
	return (
		...
		>
			...
			>
				<div className="space-y-3">
					<h1 className="font-(family-name:--font-space-grotesk) text-3xl font-bold tracking-tight text-foreground sm:text-4xl md:text-5xl">{t.blog.title}</h1>
