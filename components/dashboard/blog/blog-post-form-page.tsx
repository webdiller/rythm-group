...
export function BlogPostFormPage({ postId }: { postId?: number }) {
	...
	const coverEditorDragStartRef = useRef<{
		x: number
		y: number
		panX: number
		panY: number
	...
	const [publishedAtLocal, setPublishedAtLocal] = useState(() => unixToDatetimeLocal(Math.floor(Date.now() / 1000)))
	...
	const uploadInlineBlobImages = async (html: string): Promise<string | null> => {
		...
		for (const blobSrc of uniqueSources) {
			try {
				...
				const file = new File([blob], `inline-${Date.now()}.${ext}`, {
	...
	const coverPreviewSrc = coverDraftPreviewUrl || (trimmedCoverUrl ? (getBlogCoverSrc(trimmedCoverUrl) ?? trimmedCoverUrl) : "")
	...
	const save = async () => {
		...
		if (coverDraftFile) {
			...
			if (!isEdit && finalCoverUrl && finalCoverUrl !== uploadedCoverUrl && isManagedBlogImageRef(finalCoverUrl)) {
			...
			<main className="container mx-auto max-w-4xl px-4 py-8">
				...
				<div className="space-y-8">
					<div className="grid gap-2">
						...
						{categories.length === 0 ? <p className="text-xs text-muted-foreground">Сначала создайте категории в разделе «Блог».</p> : null}
					</div>
					...
					<div className="grid gap-2">
						...
						{!isEdit ? (
							<p className="text-xs text-muted-foreground">Для новой статьи slug подставляется из заголовка RU. После ручного правления slug кнопка выше снова строит его из заголовка и включает автоподстановку.</p>
						) : (
					...
					<div className="grid gap-2">
						...
						<SimpleEditor
							value={body_html_ru}
							onChange={setBodyHtmlRu}
							placeholder="Текст на русском…"
						/>
						<p className="text-xs text-muted-foreground">В редакторе можно добавить изображения (кнопка загрузки или вставка из буфера), а также видео (URL или загрузка mp4/webm до 100MB) и постер.</p>
					</div>
						...
						{hasCoverPreview ? (
							<div className="flex flex-col gap-3 sm:flex-row sm:items-start">
								<div className="relative inline-flex max-w-full overflow-hidden rounded-lg border border-border bg-muted/30">
									...
									/>
									{coverUploading ? <div className="absolute inset-0 flex items-center justify-center bg-background/50 text-sm font-medium text-foreground">Загрузка…</div> : null}
								</div>
					...
					<Dialog
						...
						onOpenChange={(open) => (!open ? closeCoverEditor() : setCoverEditorOpen(true))}
					>
						<DialogContent className="max-w-2xl">
							<DialogHeader>
								<DialogTitle>Редактор обложки</DialogTitle>
								<DialogDescription>Настройте масштаб и итоговый размер изображения перед загрузкой.</DialogDescription>
							</DialogHeader>
							...
							<div className="space-y-4">
								...
								<p className="text-xs text-muted-foreground">При масштабе больше 1x можно перетаскивать изображение мышью, чтобы выбрать нужную область.</p>
							</div>
							...
							<DialogFooter>
								<Button
									...
									disabled={coverEditorApplying}
								>
									Отмена
								</Button>
								<Button
									type="button"
									onClick={() => void applyCoverEditor()}
									disabled={coverEditorApplying}
								>
					...
					<div className="grid gap-2 sm:max-w-md">
						...
						/>
						<p className="text-xs text-muted-foreground">Используется для сортировки и отображения даты в карточках. При первой публикации черновика можно оставить или изменить.</p>
					</div>
