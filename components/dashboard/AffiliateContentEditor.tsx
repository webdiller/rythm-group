...
export function AffiliateContentEditor() {
  ...
  const load = async () => {
    try {
      const [heroRes, formatsRes, faqRes, settingsRes] = await Promise.all([fetch("/api/content/affiliate-hero"), fetch("/api/content/affiliate-formats"), fetch("/api/content/affiliate-faq"), fetch("/api/site/settings")])
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Форматы сотрудничества</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {formats.map((item) => (
            ...
            >
              <Input
                value={item.title_ru}
                onChange={(e) => setFormats((prev) => prev.map((x) => (x.id === item.id ? { ...x, title_ru: e.target.value } : x)))}
                placeholder="Title RU"
              />
              <Input
                value={item.title_en}
                onChange={(e) => setFormats((prev) => prev.map((x) => (x.id === item.id ? { ...x, title_en: e.target.value } : x)))}
                placeholder="Title EN"
              />
              <Textarea
                value={item.body_ru}
                onChange={(e) => setFormats((prev) => prev.map((x) => (x.id === item.id ? { ...x, body_ru: e.target.value } : x)))}
                placeholder="Body RU"
              />
              <Textarea
                value={item.body_en}
                onChange={(e) => setFormats((prev) => prev.map((x) => (x.id === item.id ? { ...x, body_en: e.target.value } : x)))}
                placeholder="Body EN"
              />
              <div className="flex items-center gap-2">
                ...
                <Switch
                  checked={!!item.hidden}
                  onCheckedChange={(v) => setFormats((prev) => prev.map((x) => (x.id === item.id ? { ...x, hidden: v } : x)))}
                />
              </div>
              <Input
                type="number"
                value={item.order_index ?? 0}
                onChange={(e) => setFormats((prev) => prev.map((x) => (x.id === item.id ? { ...x, order_index: Number(e.target.value) } : x)))}
                placeholder="Order"
              />
              <div className="md:col-span-2 flex justify-end gap-2">
                ...
                >
                  {deletingFormatId === item.id ? "Удаление..." : "Удалить"}
                </Button>
                <Button
                  disabled={deletingFormatId === item.id || savingFormatId === item.id}
                  onClick={async () => {
          ...
          <div className="flex justify-end">
            ...
            >
              {addingFormat ? "Добавление..." : "Добавить формат"}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>FAQ Affiliate</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {faq.map((item) => (
            <div
            ...
            >
              <Input
                ...
                onChange={(e) => setFaq((prev) => prev.map((x) => (x.id === item.id ? { ...x, question_ru: e.target.value } : x)))}
                placeholder="Question RU"
              />
              <Input
                value={item.question_en}
                onChange={(e) => setFaq((prev) => prev.map((x) => (x.id === item.id ? { ...x, question_en: e.target.value } : x)))}
                placeholder="Question EN"
              />
              <Textarea
                value={item.answer_ru}
                onChange={(e) => setFaq((prev) => prev.map((x) => (x.id === item.id ? { ...x, answer_ru: e.target.value } : x)))}
                placeholder="Answer RU"
              />
              <Textarea
                value={item.answer_en}
                onChange={(e) => setFaq((prev) => prev.map((x) => (x.id === item.id ? { ...x, answer_en: e.target.value } : x)))}
              ...
              <div className="flex items-center gap-2">
                ...
                <Switch
                  checked={!!item.hidden}
                  onCheckedChange={(v) => setFaq((prev) => prev.map((x) => (x.id === item.id ? { ...x, hidden: v } : x)))}
                />
              </div>
              <Input
                type="number"
                value={item.order_index ?? 0}
                onChange={(e) => setFaq((prev) => prev.map((x) => (x.id === item.id ? { ...x, order_index: Number(e.target.value) } : x)))}
                placeholder="Order"
              />
              <div className="md:col-span-2 flex justify-end gap-2">
                <Button
                ...
                >
                  {deletingFaqId === item.id ? "Удаление..." : "Удалить"}
                </Button>
                <Button
                  disabled={deletingFaqId === item.id || savingFaqId === item.id}
                  onClick={async () => {
          ...
          <div className="flex justify-end">
            ...
            >
              {addingFaq ? "Добавление..." : "Добавить FAQ"}
            </Button>
          </div>
        </CardContent>
      </Card>
