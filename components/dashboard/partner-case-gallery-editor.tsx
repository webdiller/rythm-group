...
import { getPartnerGalleryOriginalSrc, getPartnerGalleryThumbnailSrc, parsePartnerCaseGalleryJson, serializePartnerCaseGallery, type PartnerCaseGalleryImage } from "@/lib/s3/partner-gallery-url"
...
function SortableGalleryItem({ image, disabled, onDelete }: { image: PartnerCaseGalleryImage; disabled?: boolean; onDelete: () => void }) {
    ...
    >
      ...
      <div className="min-w-0 flex-1 text-xs text-muted-foreground">
        ...
        <p className="truncate">
          thumb {image.thumbnailWidth}×{image.thumbnailHeight}
        </p>
        {originalSrc ? (
          <a
            href={originalSrc}
...
export function PartnerCaseGalleryEditor({ partnerId, initialGalleryJson, onGalleryJsonChange }: PartnerCaseGalleryEditorProps) {
  const [images, setImages] = useState<PartnerCaseGalleryImage[]>(() => parsePartnerCaseGalleryJson(initialGalleryJson))
  ...
  return (
    <div className="space-y-3 rounded-md border border-border p-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground">Дополнительные изображения только для открытой страницы кейса. Original — до 2560px (WebP), thumbnail — JPG до 20 КБ. Порядок меняется перетаскиванием.</p>
      </div>
      <Input
        type="file"
