...
export function BlogPostsEditor() {
  ...
  const handleToggleStatus = async (id: number, isPublished: boolean) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(nextStatus === "published" ? "Не удалось опубликовать" : "Не удалось снять с публикации")
        return
      }
      const json = (await res.json()) as { data?: AdminPost }
      if (json.data) {
        ...
        setPosts((prev) => prev.map((post) => (post.id === id ? { ...post, status: nextStatus } : post)))
      }
      ...
      <CardContent className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
                    ...
                    <td className="py-2 pr-3">
                      <div className="flex items-center gap-2">
                        <Switch
                          ...
                          aria-label={p.status === "published" ? "Снять запись с публикации" : "Опубликовать запись"}
                        />
                        <span className="text-xs text-muted-foreground">{processingPostId === p.id && processingAction === "toggleStatus" ? "Сохранение..." : p.status === "published" ? "Опубликовано" : "Черновик"}</span>
                      </div>
                    </td>
            ...
            {posts.length === 0 ? <p className="mt-4 text-muted-foreground">Нет записей по фильтру.</p> : null}
          </div>
        )}
      </CardContent>
    </Card>
