  ...
  return (
    <div ref={setNodeRef} style={style} className="grid gap-2 rounded border p-3 md:grid-cols-2">
      ...
      <div className="space-y-2 md:col-span-2">
        ...
        >
          <SelectTrigger id={`about-card-icon-${item.id}`} className="w-full max-w-md">
      ...
      <div className="space-y-2 md:col-span-2">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          />
          {item.has_custom_icon ? (
            <Button type="button" variant="outline" size="sm" onClick={() => void onDeleteIcon(item.id)}>
              Убрать картинку
            </Button>
          ) : null}
        </div>
        <p className="text-xs text-muted-foreground">
          PNG, JPG, WebP до 5 МБ. На сайте показывается вместо Lucide. Сохраняется в WebP до 128×128.
        </p>
      </div>
...
export function AboutCardsEditor() {
  ...
  const createCard = async () => {
    if (creating) return
    if (
      createForm.title_ru.trim().length === 0 ||
      createForm.title_en.trim().length === 0 ||
      createForm.text_ru.trim().length === 0 ||
      createForm.text_en.trim().length === 0
    ) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        <p className="text-xs text-muted-foreground">
          Карточки под заголовком секции «Кто мы». Заголовок и подзаголовок секции по-прежнему в разделе «Переводы».
        ...
        {isCreateFormOpen ? (
          <div className="grid gap-3 rounded-md border p-3 md:grid-cols-2">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              >
                <SelectTrigger id="about-card-create-icon" className="w-full max-w-md">
                  <SelectValue placeholder="Иконка" />
                </SelectTrigger>
                <SelectContent className="max-h-72">
                  {ICON_OPTIONS.map((name) => (
