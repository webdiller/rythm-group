...
import { parseRelatedChannelIds, serializeRelatedChannelIds } from "@/lib/partners/related-channel-ids"
...
function SortableRelatedChannelItem({ channel, onRemove }: { channel: ChannelOption; onRemove: () => void }) {
...
export function PartnersEditor() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [partnersRes, categoriesRes] = await Promise.all([fetch("/api/content/partners"), fetch("/api/content/partner-categories")])
      if (partnersRes.ok) {
  ...
  const handleSavePartner = async (partner: Partial<Partner>, logoFile?: File | null) => {
    ...
    try {
      ...
      let body: Partial<Partner> & { id?: number } = editingPartner ? { ...partner, id: editingPartner.id } : partner
      ...
      if (!editingPartner) {
        ...
        const maxOrder = existing.length > 0 ? Math.max(...existing.map((p) => p.order_index ?? 0)) : 0
  ...
  const handleSaveCategory = async (category: Partial<PartnerCategory>) => {
    ...
    try {
      ...
      let body: Partial<PartnerCategory> & { id?: number } = editingCategory ? { ...category, id: editingCategory.id } : category
  ...
  const getPartnerList = (categoryId: number | null): Partner[] => (categoryId === null ? partners.filter((p) => p.category_id == null).sort((a, b) => a.order_index - b.order_index) : partners.filter((p) => p.category_id === categoryId).sort((a, b) => a.order_index - b.order_index))
  ...
  const partnersByCategory = categories.reduce(
    (acc, cat) => {
      acc[cat.id] = partners.filter((p) => p.category_id === cat.id).sort((a, b) => a.order_index - b.order_index)
      return acc
    },
    {} as Record<number, Partner[]>,
  )
  const uncategorizedPartners = partners.filter((p) => p.category_id == null).sort((a, b) => a.order_index - b.order_index)
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="space-y-3">
              {categories.map((cat, index) => (
                ...
                >
                  <div>
                    ...
                    <span className="text-sm text-muted-foreground ml-2">(порядок: {cat.order_index})</span>
                  </div>
              ...
              {categories.length === 0 && <p className="text-muted-foreground text-center py-4">Нет категорий. Вы можете добавить кейс без категории.</p>}
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          >
            ...
            <DialogContent className="flex max-h-[min(94vh,1040px)] w-[calc(100vw-1rem)] max-w-6xl flex-col gap-0 p-0 sm:w-full sm:max-w-6xl lg:max-w-7xl">
              <DialogHeader className="border-b px-4 py-3 sm:px-6 sm:py-4">
                <DialogTitle>{editingPartner ? "Редактировать кейс" : "Добавить кейс"}</DialogTitle>
              </DialogHeader>
              <div className="overflow-y-auto px-4 pt-4 sm:px-6">
                <PartnerForm
                  ...
                  onGalleryJsonChange={(caseGalleryJson) => {
                    ...
                    setPartners((prev) => prev.map((item) => (item.id === next.id ? { ...item, case_gallery: caseGalleryJson } : item)))
                  }}
                  onCancel={() => {
                    setIsPartnerDialogOpen(false)
                    setEditingPartner(null)
                  ...
                  {list.map((partner, index) => (
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <span className="text-sm text-muted-foreground">(порядок: {partner.order_index})</span>
                      </div>
                      <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                        <div className="flex flex-col gap-1">
                          <Button
        ...
        {partners.length === 0 && (
          <Card>
            <CardContent className="py-8">
              <p className="text-muted-foreground text-center">Нет кейсов. Сначала добавьте категорию, затем кейсы.</p>
            </CardContent>
          </Card>
        )}
      </div>
...
function CategoryForm({ category, onSave, submitting, onCancel }: { category: PartnerCategory | null; onSave: (category: Partial<PartnerCategory>) => void; submitting: boolean; onCancel: () => void }) {
...
function PartnerForm({
  ...
  useEffect(() => {
    ...
    const loadChannels = async () => {
      try {
        ...
        const rows = [...(json.data ?? [])].sort((a, b) => (a.order_index ?? 0) - (b.order_index ?? 0) || a.id - b.id)
  ...
  const relatedChannelSensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 6 } }), useSensor(KeyboardSensor, { coordinateGetter: sortableKeyboardCoordinates }))
  ...
  const caseDetailUrl = isEditingExistingPartner ? wishlistsCasePath(buildAffiliateCaseSlug({ id: partner.id, name: formData.name || partner.name })) : ""
  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        <Select
          value={formData.category_id != null ? String(formData.category_id) : NO_CATEGORY_VALUE}
          onValueChange={(value) =>
            setFormData({
              ...formData,
        ...
        <div className="flex items-center justify-between rounded border p-3">
          <Label className="flex w-full items-center justify-between">
            <div className="pr-4">
              ...
              <p className="text-xs font-normal text-muted-foreground">Значение выше сохраняется в любом случае.</p>
            </div>
            <Switch
        ...
        <div className="flex items-center justify-between rounded border p-3">
          <Label className="flex w-full items-center justify-between">
            <div className="pr-4">
              ...
              <p className="text-xs font-normal text-muted-foreground">Если выключено — на детальной странице остаются только изображения галереи (без аватарки рядом с названием).</p>
            </div>
            <Switch
              checked={formData.show_logo_on_case_detail}
              onCheckedChange={(checked) => setFormData({ ...formData, show_logo_on_case_detail: checked })}
            />
          </Label>
        </div>
      </div>
      <div className="space-y-3 rounded-md border border-border p-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Отметьте каналы из «Наши каналы», затем перетащите выбранные, чтобы задать порядок на детальной странице.</p>
        </div>
        {channelOptions.length === 0 ? (
          ...
          <div className="grid gap-4 lg:grid-cols-2">
                ...
                {channelOptions.map((channel) => {
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        checked={checked}
                        onCheckedChange={(value) => toggleRelatedChannel(channel.id, value === true)}
                      />
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-md bg-primary/10 text-xs font-bold text-primary">
                        {avatarSrc ? (
                          <img
            ...
            <div className="space-y-2">
              <p className="text-xs font-medium text-muted-foreground">Выбранные (перетащите для порядка)</p>
              {relatedChannelIds.length === 0 ? (
                <p className="rounded-md border border-dashed border-border/70 px-3 py-6 text-center text-xs text-muted-foreground">Пока ничего не выбрано</p>
              ) : (
                <DndContext
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              ...
              {selectedBlogUrl && !blogPosts.some((post) => `/blog/${post.category_slug}/${post.slug}` === selectedBlogUrl) && <SelectItem value={selectedBlogUrl}>{selectedBlogUrl}</SelectItem>}
              {blogPosts.map((post) => {
          ...
          {targetType === "blog_post" && !selectedBlogUrl && <p className="text-xs text-destructive">Выберите статью, чтобы сохранить этот вариант перехода.</p>}
        </div>
      </div>
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">Опционально. Если заполнено — на детальной странице кейса в блоке «О игре» появится кнопка «Открыть в Steam».</p>
      </div>
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">Опционально. Если заполнено — карточка в блоке «Издатели в Steam» станет кликабельной и покажет «Открыть в Steam». Если пусто — только логотип и название, без ссылки и без этой фразы.</p>
      </div>
      <div className="grid gap-2">
        <div className="flex items-center justify-between rounded border p-3">
        ...
        <div className="flex items-center justify-between rounded border p-3">
          <Label className="flex items-center justify-between w-full">
            <div className="pr-4">
              ...
              <p className="text-xs font-normal text-muted-foreground">Логотип и название. Ссылка — только если заполнено поле «Ссылка издателя в Steam».</p>
            </div>
            <Switch
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Одна картинка используется на главной, в кейсах Wishlists и в Steam-блоке. Рекомендуется квадрат или горизонталь от 800px, PNG/WebP с прозрачным фоном. Файл сжимается до WebP (до 5 МБ), без сильного даунскейла. У уже созданной карточки можно заменить или удалить изображение без
          пересоздания кейса.
        </p>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="relative flex h-28 w-40 items-center justify-center overflow-hidden rounded-md border border-border bg-muted">
            {hasLogo && partner ? (
              <img
                key={logoVersion}
                src={getPartnerLogoSrc({ id: partner.id, logo_url: logoKey }, { cacheBust: logoVersion }) ?? undefined}
                alt={partner.name}
            ...
            {logoUploading ? <div className="absolute inset-0 flex items-center justify-center bg-background/50 text-xs font-medium text-foreground">Загрузка…</div> : null}
          </div>
          {partner && (
        ...
        {!partner && (
          <div className="mt-3 flex flex-col gap-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {newLogoFile ? (
                <>
                  <span className="max-w-[14rem] truncate text-xs text-muted-foreground">{newLogoFile.name}</span>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
      ...
      {partner ? (
        ...
        <p className="text-xs text-muted-foreground">Галерею детальной страницы можно заполнить после сохранения кейса.</p>
      )}
