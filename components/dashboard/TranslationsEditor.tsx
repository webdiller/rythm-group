...
export function TranslationsEditor() {
	...
	const loadTranslations = async () => {
		setLoading(true)
		try {
			const response = await fetch(`/api/content/translations?locale=${selectedLocale}&section=${selectedSection}`)
			if (response.ok) {
	...
	const saveOneTranslation = async (key: string, value: string): Promise<boolean> => {
		...
		const body = existingId != null ? { id: existingId, locale: selectedLocale, section: selectedSection, key, value } : { locale: selectedLocale, section: selectedSection, key, value }
	...
	const getChangedKeys = (): string[] => {
		...
		return Array.from(keys).filter((key) => (translations[key]?.value ?? "") !== (savedTranslations[key]?.value ?? ""))
	}
	...
	const parseStatsItems = (raw: string): Array<{ value: string; label: string }> => {
		if (!raw) return []
		try {
	...
	const getNestedKeys = (section: string): string[] => {
		...
		const keysMap: Record<string, string[]> = {
			...
			contact: ["title", "subtitle", "name", "email", "company", "message", "budget", "budgetOptions", "submit", "or", "telegram", "emailUs"],
			footer: ["rights", "description", "ctaTitle", "privacy", "dataPolicy"],
			blog: ["title", "subtitle", "breadcrumbHome", "allCategories", "readMore", "backToBlog", "emptyAll", "dateLabel"],
			affiliate: [
				"formats.title",
				"formats.subtitle",
				"cases.title",
		...
		<div className="space-y-6">
			<div className="flex gap-4">
				<Select
					...
					onValueChange={(value) => setSelectedLocale(value as "ru" | "en")}
				>
								...
								>
									...
									<div className="space-y-3">
										...
										<p className="text-xs text-muted-foreground">
											Элементы будут сохранены как JSON-массив объектов с полями <code>value</code> и <code>label</code>.
										</p>
									</div>
								</div>
							)
						...
						return (
							...
							>
								...
								<Textarea
									...
									rows={key.includes("text") || key.includes("subtitle") || key === "pageDescription" ? 3 : 1}
								/>
							</div>
						)
					})}
