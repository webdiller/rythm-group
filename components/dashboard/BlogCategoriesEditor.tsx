...
function SortableCategoryRow({
  ...
  return (
    <tr ref={setNodeRef} style={style} className="border-b border-border/60">
      ...
      <td className="py-2">
        <div className="flex gap-1">
          ...
          <Button variant="ghost" size="icon" onClick={() => onDelete(category.id)} aria-label="Delete">
...
export function BlogCategoriesEditor() {
  ...
  const activeRows = useMemo(
    () =>
      [...rows.filter((r) => !r.deleted_at)].sort(
  ...
  const openCreate = () => {
    setEditing(null)
    setForm({ slug: "", name_ru: "", name_en: "", order_index: rows.filter((r) => !r.deleted_at).length })
    setDialogOpen(true)
  }
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Перетащите строки за иконку слева, чтобы изменить порядок на сайте.
          </p>
        </div>
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="overflow-x-auto">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-muted-foreground">
                    <th className="w-10 pb-2 pr-2" aria-hidden />
            ...
            {savingOrder ? (
              <p className="mt-2 text-xs text-muted-foreground">Сохранение порядка…</p>
            ) : null}
            {activeRows.length === 0 ? (
              <p className="mt-4 text-muted-foreground">Нет категорий. Добавьте первую или выполните seed БД.</p>
            ) : null}
          </div>
        )}
        ...
        <AlertDialog open={deleteId != null} onOpenChange={(o) => !o && setDeleteId(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Категория и все её записи будут скрыты (soft delete). Публичный сайт перестанет их показывать.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
