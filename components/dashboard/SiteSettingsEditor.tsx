...
function SortableNavItem({ id, labelRu, labelEn }: { id: HeaderNavItemId; labelRu: string; labelEn: string }) {
...
export function SiteSettingsEditor() {
  const { t } = useLocale()
  const [headerNavLabels, setHeaderNavLabels] = useState<Record<HeaderNavItemId, { ru: string; en: string }>>({
    about: { ru: fallbackTranslations.ru.nav.about, en: fallbackTranslations.en.nav.about },
    channels: {
      ru: fallbackTranslations.ru.nav.channels,
      en: fallbackTranslations.en.nav.channels,
    },
    cases: { ru: fallbackTranslations.ru.nav.cases, en: fallbackTranslations.en.nav.cases },
    affiliate: {
      ru: fallbackTranslations.ru.nav.affiliate,
      en: fallbackTranslations.en.nav.affiliate,
    },
    blog: {
    ...
    contacts: {
      ru: fallbackTranslations.ru.nav.contacts,
      en: fallbackTranslations.en.nav.contacts,
    },
  })
  ...
  const [partnersDisplayMode, setPartnersDisplayMode] = useState<"name" | "logo" | "logoAndName">("logoAndName")
  ...
  useEffect(() => {
    ...
    const applyRows = (locale: "ru" | "en", rows: Array<{ section: string; key: string; value: string }>, next: Record<HeaderNavItemId, { ru: string; en: string }>) => {
    ...
    const loadNavLabels = async () => {
      try {
        const [ruRes, enRes] = await Promise.all([fetch("/api/content/translations?locale=ru", { cache: "no-store" }), fetch("/api/content/translations?locale=en", { cache: "no-store" })])
        if (!ruRes.ok || !enRes.ok) return
        const ruJson = (await ruRes.json()) as {
  ...
  const bgPreview = (kind: "hero" | "global", theme?: "light" | "dark" | null, scope?: "affiliate" | null, cacheBust?: number) => {
  ...
  const rememberBackgroundKey = (kind: "hero" | "global", theme: string | null | undefined, scope: string | null | undefined, key: string | null | undefined) => {
    const slot = resolveBackgroundSlot(kind, theme, scope)
  ...
  useEffect(() => {
    const loadSettings = async () => {
      ...
      try {
        ...
        const channelsAlign = data?.channels_card_align === "center" || data?.channels_card_align === "right" ? data.channels_card_align : "left"
  ...
  const handleUploadHeroBackgroundWithTheme = async (file: File, theme: "light" | "dark", scope: "landing" | "affiliate" = "landing") => {
    const maxSizeBytes = 5 * 1024 * 1024
    if (file.size > maxSizeBytes) {
      toast.error("Файл не должен превышать 5 МБ")
      return
    ...
    const setUploadingFn = scope === "affiliate" ? (theme === "light" ? setUploadingAffiliateHeroBackgroundLight : setUploadingAffiliateHeroBackgroundDark) : theme === "light" ? setUploadingHeroBackgroundLight : setUploadingHeroBackgroundDark
    ...
    try {
      ...
      toast.success(`${scope === "affiliate" ? "Фон hero (/affiliate)" : "Фон hero"} ${theme === "light" ? "для светлой темы обновлён" : "для тёмной темы обновлён"}`)
    } catch {
      notifyMutationError("Не удалось загрузить фон для hero")
    } finally {
  ...
  const handleDeleteHeroBackgroundWithTheme = async (theme: "light" | "dark", scope: "landing" | "affiliate" = "landing") => {
    const setDeletingFn = scope === "affiliate" ? (theme === "light" ? setDeletingAffiliateHeroBackgroundLight : setDeletingAffiliateHeroBackgroundDark) : theme === "light" ? setDeletingHeroBackgroundLight : setDeletingHeroBackgroundDark
    ...
    try {
      ...
      toast.success(`${scope === "affiliate" ? "Фон hero (/affiliate)" : "Фон hero"} ${theme === "light" ? "для светлой темы сброшен" : "для тёмной темы сброшен"}`)
    } catch {
      notifyMutationError("Не удалось удалить фон для hero")
    } finally {
      setDeletingFn(false)
  ...
  const handleUploadGlobalBackgroundWithTheme = async (file: File, theme: "light" | "dark", scope: "landing" | "affiliate" = "landing") => {
    const maxSizeBytes = 5 * 1024 * 1024
    if (file.size > maxSizeBytes) {
      toast.error("Файл не должен превышать 5 МБ")
      return
    ...
    const setUploadingFn = scope === "affiliate" ? (theme === "light" ? setUploadingAffiliateGlobalBackgroundLight : setUploadingAffiliateGlobalBackgroundDark) : theme === "light" ? setUploadingGlobalBackgroundLight : setUploadingGlobalBackgroundDark
    ...
    try {
      ...
      toast.success(`${scope === "affiliate" ? "Общий фон (/affiliate)" : "Общий фон"} ${theme === "light" ? "для светлой темы обновлён" : "для тёмной темы обновлён"}`)
    } catch {
      notifyMutationError("Не удалось загрузить общий фон")
    } finally {
      setUploadingFn(false)
  ...
  const handleDeleteGlobalBackgroundWithTheme = async (theme: "light" | "dark", scope: "landing" | "affiliate" = "landing") => {
    const setDeletingFn = scope === "affiliate" ? (theme === "light" ? setDeletingAffiliateGlobalBackgroundLight : setDeletingAffiliateGlobalBackgroundDark) : theme === "light" ? setDeletingGlobalBackgroundLight : setDeletingGlobalBackgroundDark
    ...
    try {
      ...
      toast.success(`${scope === "affiliate" ? "Общий фон (/affiliate)" : "Общий фон"} ${theme === "light" ? "для светлой темы сброшен" : "для тёмной темы сброшен"}`)
    } catch {
      notifyMutationError("Не удалось удалить общий фон")
    } finally {
      setDeletingFn(false)
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-xs text-muted-foreground">{hasLogo ? "Кастомный логотип загружен" : "Используется логотип по умолчанию"}</span>
            </div>
            <p className="text-xs text-muted-foreground">Загружается PNG/JPG/WebP до 5 МБ. Используется в `Header` и `Footer`.</p>
          </div>
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">Если оставить пустым, текст рядом с логотипом в `Header` и `Footer` не отображается.</p>
          </div>
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded bg-muted border border-border">
                {hasFavicon ? (
                  ...
                  <span className="text-[10px] text-muted-foreground text-center px-1">Используется фавикон по умолчанию</span>
                )}
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              Загрузите PNG/WebP/SVG логотип. Он будет автоматически преобразован в квадратный фавикон 32×32 px. Если удалить кастомный фавикон, вернётся значение по умолчанию (<code>/logo.jpg</code>).
            </p>
          </div>
          <div className="space-y-2">
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Включает или отключает анимацию световых лучей и появления элементов на сайте.</p>
            </div>
            <Switch
          ...
          <div className="space-y-3 border-t border-border/60 pt-4">
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Используется, когда включена светлая тема. Если не задан, берётся основной фон Hero.</p>
              </div>
              <div className="flex h-16 w-28 items-center justify-center overflow-hidden rounded border border-border bg-muted">
                {hasHeroBackgroundLight ? (
                  ...
                  <span className="px-2 text-center text-[10px] text-muted-foreground">Фон hero для светлой темы не загружен</span>
                )}
              </div>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-3 border-t border-border/60 pt-4">
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Используется, когда включена тёмная тема. Если не задан, берётся основной фон Hero.</p>
              </div>
              <div className="flex h-16 w-28 items-center justify-center overflow-hidden rounded border border-border bg-muted">
                {hasHeroBackgroundDark ? (
                  ...
                  <span className="px-2 text-center text-[10px] text-muted-foreground">Фон hero для тёмной темы не загружен</span>
                )}
              </div>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-3 border-t border-border/60 pt-4">
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Используется, когда включена светлая тема. Если не задан, берётся основной общий фон.</p>
              </div>
              <div className="flex h-16 w-28 items-center justify-center overflow-hidden rounded border border-border bg-muted">
                {hasGlobalBackgroundLight ? (
                  ...
                  <span className="px-2 text-center text-[10px] text-muted-foreground">Общий фон для светлой темы не загружен</span>
                )}
              </div>
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-3 border-t border-border/60 pt-4">
            <div className="flex items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">Используется, когда включена тёмная тема. Если не задан, берётся основной общий фон.</p>
              </div>
              <div className="flex h-16 w-28 items-center justify-center overflow-hidden rounded border border-border bg-muted">
                {hasGlobalBackgroundDark ? (
                  ...
                  <span className="px-2 text-center text-[10px] text-muted-foreground">Общий фон для тёмной темы не загружен</span>
                )}
              </div>
            </div>
            <div className="space-y-2">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <p className="text-xs text-muted-foreground">Отдельные фоновые изображения для страницы Affiliate. Если не загружены, используется визуальный фон по умолчанию.</p>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => setPartnersDisplayMode(e.target.value as "name" | "logo" | "logoAndName")}
            >
            ...
            <p className="text-xs text-muted-foreground">Определяет, как партнёры будут отображаться на лендинге в блоке кейсов.</p>
          </div>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setChannelsCardAlign(value === "center" || value === "right" ? value : "left")
              }}
            >
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Если выключено, число подписчиков не отображается на карточках.</p>
            </div>
            <Switch
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Если выключено, охват не отображается на карточках.</p>
            </div>
            <Switch
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-primary"
              value={contactLayout}
              onChange={(e) => setContactLayout(e.target.value === "contactsFirst" ? "contactsFirst" : "formFirst")}
            >
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Если включено, на лендинге будут отображаться только ссылки на Telegram и другие соцсети.</p>
            </div>
            <Switch
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) => setAffiliateContactLayout(e.target.value === "contactsFirst" ? "contactsFirst" : "formFirst")}
            >
              <option value="formFirst">Сначала форма, затем соцсети</option>
              <option value="contactsFirst">Сначала соцсети, затем форма</option>
            </select>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Если включено, на странице Affiliate будут отображаться только ссылки на Telegram и другие соцсети.</p>
            </div>
            <Switch
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Даты на карточках и на странице записи на публичном сайте.</p>
            </div>
            <Switch
              id="blog_show_dates"
              checked={blogShowDates}
              onCheckedChange={setBlogShowDates}
            />
          </div>
          {/* <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">Перетащите пункты для изменения порядка кнопок разделов в `Header`.</p>
          <DndContext
            sensors={sensors}
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">{sitePublished ? "Сайт доступен для посетителей." : "Сайт в режиме обслуживания — публичные страницы недоступны."}</p>
            </div>
            <Switch
              id="site_published"
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">Укажите полный URL на страницу с политикой конфиденциальности. Это может быть внешний сайт.</p>
          </div>
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">Укажите полный URL на страницу с политикой обработки данных. Это может быть внешний сайт.</p>
          </div>
        </CardContent>
      </Card>
