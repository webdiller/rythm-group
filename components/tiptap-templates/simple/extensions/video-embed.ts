...
export const VideoEmbed = Node.create({
  ...
  addAttributes() {
    return {
      src: {
        default: "",
        parseHTML: (element) => element.getAttribute("data-video-src") ?? element.getAttribute("src") ?? "",
      },
      poster: {
        default: null,
        parseHTML: (element) => element.getAttribute("data-video-poster") ?? element.getAttribute("poster") ?? null,
      },
    }
  },

  parseHTML() {
    return [{ tag: "figure[data-video-embed]" }, { tag: "video[data-video-embed]" }]
  },
