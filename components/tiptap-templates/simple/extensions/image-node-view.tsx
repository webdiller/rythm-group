...
export function ImageNodeView({ node, updateAttributes, deleteNode, selected }: ImageNodeViewProps) {
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    dragStartRef.current = { x: event.clientX, y: event.clientY, panX: editorPanX, panY: editorPanY }
  }
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 right-2 z-20 flex items-center gap-1 rounded-md border border-border/70 bg-background/90 p-1 opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100">
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Уменьшить" onClick={() => setWidthDelta(-10)}>
          <Minus className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Увеличить" onClick={() => setWidthDelta(10)}>
          <Plus className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Влево" onClick={() => setAlign("left")}>
          <AlignLeft className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="По центру" onClick={() => setAlign("center")}>
          <AlignCenter className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Вправо" onClick={() => setAlign("right")}>
          <AlignRight className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Редактировать (crop/zoom)" onClick={() => void reopenEditorFromCurrent()}>
          <Crop className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Заменить файлом" onClick={() => inputRef.current?.click()}>
          <Upload className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7" title="Заменить по URL" onClick={replaceByUrl}>
          <Link2 className="h-3.5 w-3.5" />
        </Button>
        <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" title="Удалить изображение" onClick={() => deleteNode()}>
      ...
      <Dialog open={editorOpen} onOpenChange={(open) => (!open ? closeEditor() : setEditorOpen(true))}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-2">
                ...
                <Input id="inline-image-width" type="number" min={100} max={2400} value={editorWidth} onChange={(e) => setEditorWidth(Number(e.target.value) || 0)} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="inline-image-height">Высота, px</Label>
                <Input id="inline-image-height" type="number" min={100} max={2400} value={editorHeight} onChange={(e) => setEditorHeight(Number(e.target.value) || 0)} />
              </div>
            </div>
            <div className="grid gap-2">
              ...
              <Slider min={1} max={3} step={0.01} value={[editorScale]} onValueChange={(value) => {
            ...
            {editorImageUrl ? (
              <div
                ...
                style={{ width: "100%", maxWidth: "560px", aspectRatio: `${Math.max(100, editorWidth)} / ${Math.max(100, editorHeight)}` }}
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={() => void applyLocalImage()}>Применить локально</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </NodeViewWrapper>
