...
export function SimpleEditor({ value, onChange, placeholder = "Начните ввод…", className }: SimpleEditorProps) {
  ...
  const imageEditorDragStartRef = useRef<{ x: number; y: number; panX: number; panY: number } | null>(null)
  ...
  const getSelectedImageAttrs = (): { src: string; width: number; align: "left" | "center" | "right" } | null => {
    if (!editor.isActive("image")) return null
    const attrs = editor.getAttributes("image") as { src?: string; width?: number; align?: "left" | "center" | "right" }
  ...
  const uploadInlineImageFile = async (file: File | null) => {
    ...
    try {
      ...
      editor.chain().focus().insertContent({ type: "image", attrs: { src: localUrl } }).run()
    ...
    <div className={cn("rounded-lg border border-border bg-card overflow-hidden", className)}>
      <div className="flex flex-wrap items-center gap-0.5 border-b border-border bg-muted/40 px-1 py-1">
        ...
        {selectedImage ? (
          <>
            <Separator orientation="vertical" className="mx-0.5 h-6" />
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Crop/Zoom (локально)" onClick={() => void openImageEditorFromSrc(selectedImage.src)}>
              <Crop className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Шире" onClick={() => setSelectedImageWidthDelta(10)}>
              <Plus className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Уже" onClick={() => setSelectedImageWidthDelta(-10)}>
              <Minus className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Выравнивание влево" onClick={() => setSelectedImageAlign("left")}>
              <AlignLeft className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Выравнивание по центру" onClick={() => setSelectedImageAlign("center")}>
              <AlignCenter className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Выравнивание вправо" onClick={() => setSelectedImageAlign("right")}>
              <AlignRight className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Заменить выбранное изображение файлом" onClick={() => selectedImageInputRef.current?.click()}>
              <Upload className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Заменить выбранное изображение по URL" onClick={replaceSelectedImageByUrl}>
              <Link2 className="h-4 w-4" />
            </Button>
            <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" title="Удалить выбранное изображение" onClick={removeSelectedImage}>
              <Trash2 className="h-4 w-4" />
            </Button>
          </>
        ) : null}
      ...
      <Dialog open={imageEditorOpen} onOpenChange={(open) => (!open ? closeImageEditor() : setImageEditorOpen(true))}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>Редактор изображения</DialogTitle>
            <DialogDescription>
              Настройте масштаб и размер. При масштабе больше 1x можно перетаскивать изображение.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-2">
                ...
                <Input id="selected-image-width" type="number" min={100} max={2400} value={imageEditorWidth} onChange={(e) => setImageEditorWidth(Number(e.target.value) || 0)} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="selected-image-height">Высота, px</Label>
                <Input id="selected-image-height" type="number" min={100} max={2400} value={imageEditorHeight} onChange={(e) => setImageEditorHeight(Number(e.target.value) || 0)} />
              </div>
            </div>
            <div className="grid gap-2">
              ...
              <Slider min={1} max={3} step={0.01} value={[imageEditorScale]} onValueChange={(value) => {
            ...
            {imageEditorImageUrl ? (
              <div
                ...
                style={{ width: "100%", maxWidth: "560px", aspectRatio: `${Math.max(100, imageEditorWidth)} / ${Math.max(100, imageEditorHeight)}` }}
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={closeImageEditor}>Отмена</Button>
            <Button type="button" onClick={() => void applyImageEditor()}>Применить локально</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
