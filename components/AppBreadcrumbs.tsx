...
export function AppBreadcrumbs({ items, className, navClassName, currentPageClassName, "aria-label": ariaLabel = "breadcrumb" }: AppBreadcrumbsProps) {
  if (items.length === 0) return null

  return (
    <Breadcrumb
      aria-label={ariaLabel}
      className={cn(DEFAULT_NAV_CLASS, navClassName)}
    >
        ...
        {items.map((item, i) => {
          ...
          return (
            <Fragment key={`${i}-${item.label}`}>
              ...
              <BreadcrumbItem>
                {isLast ? (
                  <BreadcrumbPage className={cn("max-w-48 truncate sm:max-w-none", currentPageClassName)}>{item.label}</BreadcrumbPage>
                ) : (
