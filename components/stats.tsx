      ...
      <div className="relative mx-auto max-w-7xl">
        <ScrollReveal disabled={!animationsEnabled}>
          <div className="mb-12 text-center md:mb-16">
            <h2 className="mb-4 text-3xl font-bold tracking-tight text-foreground md:text-5xl text-balance">{t.stats.title}</h2>
            <p className="mx-auto max-w-2xl text-base text-muted-foreground md:text-lg text-pretty">{t.stats.subtitle}</p>
          </div>
        </ScrollReveal>
          ...
          {t.stats.items.map((item, i) => (
            ...
            >
              <div className="flex flex-col items-center rounded-xl border border-border bg-card p-8 text-center transition-all hover:border-primary/30 glow-border">
                <span className="mb-2 text-3xl font-bold text-primary md:text-4xl lg:text-5xl text-glow">{item.value}</span>
