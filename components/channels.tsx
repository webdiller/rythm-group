...
export function Channels({ categories, channels, animationsEnabled = true, showSubscribers = true, showReach = true, cardAlign = "left" }: ChannelsProps) {
  ...
  const activeChannels = activeCategoryId ? channels.filter((c) => c.category_id === activeCategoryId).sort((a, b) => a.order_index - b.order_index) : []

  const uncategorizedChannels = channels.filter((c) => !c.category_id).sort((a, b) => a.order_index - b.order_index)
  ...
  const visibleUncategorized = showAllUncategorized ? uncategorizedChannels : uncategorizedChannels.slice(0, MAX_VISIBLE_UNCATEGORIZED)
      ...
      <div className="mx-auto max-w-7xl">
        <ScrollReveal disabled={!animationsEnabled}>
          <div className={`text-center ${categories.length === 0 ? "mb-6 md:mb-8" : "mb-12 md:mb-16"}`}>
            <h2 className="mb-4 text-3xl font-bold tracking-tight text-foreground md:text-5xl text-balance">{t.channels.title}</h2>
            <p className="mx-auto max-w-2xl text-base text-muted-foreground md:text-lg text-pretty">{t.channels.subtitle}</p>
          </div>
        </ScrollReveal>

        {uncategorizedChannels.length > 0 && (
          <div className="mt-4 mb-10">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visibleUncategorized.map((channel, index) => renderCard(channel, index))}</div>
            {uncategorizedChannels.length > MAX_VISIBLE_UNCATEGORIZED && (
              <div className="mt-8 flex justify-center">
                ...
                >
                  {showAllUncategorized ? (locale === "ru" ? `Свернуть список (${uncategorizedHiddenCount})` : `Show less (${uncategorizedHiddenCount})`) : locale === "ru" ? `Показать ещё ${uncategorizedHiddenCount}` : `Show ${uncategorizedHiddenCount} more`}
                </button>
              </div>
            )}
          </div>
        ...
        {categories.length > 0 && (
          <>
                  ...
                  {categories.map((cat) => (
                    <button
                      ...
                      className={`rounded-lg px-5 py-2.5 text-sm font-medium transition-all ${
                        activeCategoryId === cat.id ? "bg-primary text-primary-foreground shadow-[0_0_20px_rgba(230,27,0,0.2)]" : "border border-border bg-secondary/50 text-muted-foreground hover:bg-secondary hover:text-foreground"
                      }`}
                    >
            ...
            {activeChannels.length === 0 && uncategorizedChannels.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">{locale === "ru" ? "Каналы не найдены." : "No channels found."}</div>
            ) : activeChannels.length > 0 ? (
              <>
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{visibleChannels.map((channel, index) => renderCard(channel, index))}</div>
        ...
        {categories.length === 0 && uncategorizedChannels.length === 0 && <div className="text-center py-12 text-muted-foreground">{locale === "ru" ? "Каналы не найдены." : "No channels found."}</div>}
      </div>
    </section>
  )
}

function ChannelCard({ channel, showSubscribers, showReach, cardAlign, subscribersLabel, reachLabel }: { channel: Channel; showSubscribers: boolean; showReach: boolean; cardAlign: ChannelsCardAlign; subscribersLabel: string; reachLabel: string }) {
  const showMeta = (showSubscribers && Boolean(channel.subscribers)) || (showReach && Boolean(channel.reach))
  ...
  return (
    ...
    >
      ...
      <div className={cn(cardAlign === "center" && "flex flex-col items-center")}>
        ...
        {showMeta ? (
          <div className={cn("mt-1.5 flex flex-col gap-1 text-xs text-muted-foreground", cardAlign === "center" && "items-center", cardAlign === "right" && "items-end")}>
...
function ChannelAvatar({ channelId, name, avatar, hasAvatar }: { channelId: number; name: string; avatar?: string | null; hasAvatar?: boolean }) {
