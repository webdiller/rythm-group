...
export function Footer({ siteSettings, sectionHrefPrefix = "", pageBlogEnabled, pageAffiliateEnabled }: FooterProps) {
	...
	const navOrder = (() => {
		try {
			return normalizeHeaderNavOrder(siteSettings?.headerNavOrder ? JSON.parse(siteSettings.headerNavOrder) : undefined)
		} catch {
	...
	const handleNavClick = (rawHref: string) => createAnchorClickHandler({ rawHref, sectionHrefPrefix })
