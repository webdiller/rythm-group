...
function getHomeData(): {
  ...
  const channelCategories = [...rawCategories].map((c) => ({ ...c, order_index: c.order_index ?? 0 })).sort((a, b) => a.order_index - b.order_index) as ChannelCategory[]
  ...
  const partnerCategories = [...rawPartnerCategories].map((c) => ({ ...c, order_index: c.order_index ?? 0 })).sort((a, b) => a.order_index - b.order_index) as PartnerCategory[]
...
export default async function Home() {
  ...
  const { channelCategories, channels, partnerCategories, partners, siteSettings, blogPosts, blogCategories, blogShowDates, headerNavOrder, aboutCards } = data

  const animationsEnabled = siteSettings?.heroAnimationEnabled ?? true
  const contactLayout = (siteSettings?.contactLayout as "formFirst" | "contactsFirst" | null) ?? "formFirst"
  const contactFormHidden = siteSettings?.contactFormHidden ?? false
  const partnersDisplayMode = (siteSettings?.partnersDisplayMode as "name" | "logo" | "logoAndName" | null) ?? "logoAndName"
  ...
  const channelsCardAlign = siteSettings?.channels_card_align === "center" || siteSettings?.channels_card_align === "right" ? siteSettings.channels_card_align : "left"
