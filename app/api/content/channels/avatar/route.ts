...
import { deleteChannelAvatarIfStored, uploadChannelAvatarWebp } from "@/lib/s3/channel-avatar"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const optimizedBuffer = await sharp(inputBuffer).resize(256, 256, { fit: "cover" }).webp({ quality: 80 }).toBuffer()

    const db = getDb()
    const existing = db.select({ avatar: tableChannels.avatar }).from(tableChannels).where(eq(tableChannels.id, channelId)).get()
    ...
    const [updated] = db.update(tableChannels).set({ avatar: key }).where(eq(tableChannels.id, channelId)).returning().all()
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const [updated] = db.update(tableChannels).set({ avatar: null }).where(eq(tableChannels.id, channelId)).returning().all()
