...
export async function POST(
  ...
  try {
    ...
    if (!alreadySeen) {
      db.insert(tableAffiliatePartnerViews).values({
    ...
    const row = db
      .select({ views: tablePartners.views })
      .from(tablePartners)
      .where(eq(tablePartners.id, partnerId))
      .get()

    return NextResponse.json({ data: { views: row?.views ?? 0, incremented: !alreadySeen }, meta: null })
  } catch (error) {
