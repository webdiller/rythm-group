...
import { deletePartnerLogoIfStored, uploadPartnerLogoWebp } from "@/lib/s3/partner-logo"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const db = getDb()
    const existing = db.select({ logo_url: tablePartners.logo_url }).from(tablePartners).where(eq(tablePartners.id, partnerId)).get()
    ...
    const [updated] = db.update(tablePartners).set({ logo_url: key }).where(eq(tablePartners.id, partnerId)).returning().all()
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const [updated] = db.update(tablePartners).set({ logo_url: null }).where(eq(tablePartners.id, partnerId)).returning().all()
