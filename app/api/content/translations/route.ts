...
export async function PUT(request: NextRequest) {
  try {
    requireAuth(request)
    const body = await request.json() as { id?: unknown; locale?: string; section?: string; key?: string; value?: string }
    const { id, ...rest } = body
    if (id == null) {
    ...
    if (!paramsParsed.success || !bodyParsed.success) {
      return NextResponse.json(
        { errors: (paramsParsed.success ? bodyParsed.error : paramsParsed.error)?.flatten() },
        { status: 400 }
      )
    }
