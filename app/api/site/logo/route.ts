...
import { deleteSiteAssetIfStored, uploadSiteLogoWebp } from "@/lib/s3/site-assets"
...
export async function POST(request: NextRequest) {
	try {
		...
		const optimizedBuffer = await sharp(inputBuffer).resize(512, 512, { fit: "inside", withoutEnlargement: false }).webp({ quality: 85 }).toBuffer()
		...
		try {
			if (existing) {
				const [updated] = db.update(tableSiteSettings).set({ logo: key }).where(eq(tableSiteSettings.id, existing.id)).returning().all()
...
export async function DELETE(request: NextRequest) {
	try {
		...
		const [updated] = db.update(tableSiteSettings).set({ logo: null }).where(eq(tableSiteSettings.id, existing.id)).returning().all()
